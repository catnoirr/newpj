    ...
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-center p-4">
          ...
          {/* Expand/Collapse Button only appears on large screens */}
          {!isMobileMenuOpen && (
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className={`absolute right-[-14px] top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-md bg-white shadow-md flex items-center justify-center text-purple-800 z-50`}
            >
...
function SidebarItem({ icon, label, route, isExpanded, textColor = "text-white" }) {
  ...
  return (
    <div className="relative group flex justify-center">
      ...
      {!isExpanded && (
        <span className="absolute left-16 top-1/2 -translate-y-1/2 px-2 py-1 text-sm bg-purple-800 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          {label}
